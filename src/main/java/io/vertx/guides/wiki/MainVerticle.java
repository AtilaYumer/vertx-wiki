...
public class MainVerticle extends AbstractVerticle {
	...
	public void start(Future<Void> startFuture) throws Exception {
		Future<Void> steps = this.prepareDatabase().compose(v -> this.startHttpServer());
	...
	private void indexHandler(RoutingContext context) {
		dbClient.getConnection(connectionHandler -> {
			if (connectionHandler.succeeded()) {
				...
				connection.query(SQL_ALL_PAGES, queryHandler -> {
					connection.close();

					if (queryHandler.succeeded()) {
						List<String> pages = queryHandler.result().getResults().stream().map(json -> json.getString(0))
				...
				context.fail(connectionHandler.cause());
			}
		});
	}
	...
	private void pageRenderingHandler(RoutingContext context) {
		...
		dbClient.getConnection(car -> {
			if (car.succeeded()) {
				...
				connection.queryWithParams(SQL_GET_PAGE, new JsonArray().add(page), queryHandler -> {
					if (queryHandler.succeeded()) {
						connection.close();

						JsonArray row = queryHandler.result().getResults().stream().findFirst()
						...
						context.put("id", id);
						context.put("newPage", queryHandler.result().getResults().size() == 0 ? "yes" : "no");
